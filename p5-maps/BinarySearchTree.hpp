...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static Node *copy_nodes_impl(Node *node) {
    ...
    Node *newptr = new Node(node->datum, nullptr, nullptr);
    newptr->left = copy_nodes_impl(node->left);
    newptr->right = copy_nodes_impl(node->right);
  ...
  static Node * insert_impl(Node *node, const T &item, Compare less) {
    if(!node){
      Node* newptr = new Node{item, nullptr, nullptr}; 
      return newptr; 
    }
      ...
      node->right = insert_impl(node->right, item, less); 
        ...
        node->datum = item;
        return node;  // No memory leak, as we're not creating a new node
    }
